...
class PatientController extends Controller
{
    ...
    public function index()
    {
        ...
        $patients = Patient::with('reservations')->radiologyCenter()->get();

        return view('backend.dashboards.radiologyCenter.pages.patients.index', compact('patients'));
    }

    public function data()
    {
        $patients = Patient::with('medicalAnalysis')->radiologyCenter()->get();

        return DataTables::of($patients)
            ->addColumn('number_of_rays', function ($patient) {
                return $patient->rays()->count();
            })
            ->addColumn('add_rays', function ($patient) {
                return '<a href="' . route('radiologyCenter.rays.add', $patient->id) . '" 
                            class="btn btn-info btn-sm">
                            ' . trans('backend/patients_trans.Add_Rays') . '
                        </a>';
            })
           
            ->addColumn('patient_card', function ($patient) {
                return '<a href="' . route('radiologyCenter.patients.patient_pdf', $patient->id) . '" 
                            class="btn btn-primary btn-sm">
                            ' . trans('backend/patients_trans.Show_Patient_Card') . '
                        </a>';
            })
            ->addColumn('action', function ($patient) {
                $editUrl = route('radiologyCenter.patients.edit', $patient->id);
                $deleteUrl = route('radiologyCenter.patients.destroy', $patient->id);
                $viewUrl = route('radiologyCenter.patients.show', $patient->id);
            ...
            ->rawColumns(['add_rays', 'patient_card', 'action'])
            ->make(true);
    }
    ...
    public function show($id)
    {
        ...
        return view('backend.dashboards.radiologyCenter.pages.patients.show', compact('patient'));
    }
    ...
    public function patientPdf($id)
    {
        ...
        $pdf = PDF::loadView(
            'backend.dashboards.radiologyCenter.pages.patients.patient_card',
            $data,
            [],
            [
                // 'format' => 'A5-L',
    ...
    public function add()
    {
        ...
        return view('backend.dashboards.radiologyCenter.pages.patients.add');
    }
    ...
    public function store(StorePatientRequest $request)
    {
        ...
        try {
            ...
            return redirect()->route('radiologyCenter.patients.index')->with('toast_success', 'Patient added toast_successfully');
        } catch (\Exception $e) {
    ...
    public function edit($id)
    {
        ...
        return view('backend.dashboards.radiologyCenter.pages.patients.edit', compact('patient'));
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect()->route('radiologyCenter.patients.index');
    }
    ...
    public function trash()
    {
        ...
        return view('backend.dashboards.radiologyCenter.pages.patients.trash', compact('patients'));
    }
    ...
    {
        return view('backend.dashboards.radiologyCenter.pages.patients.add_patient_code');
    }
