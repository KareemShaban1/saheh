...
<script>
   let typeTable = $('#type_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('medicalLaboratory.type.data') }}",
        columns: [{
                data: 'id',
                name: 'id'
            },
    ...
    function resetForm() {
        $('#TypeForm')[0].reset();
        $('#TypeForm').attr('action', '{{ route("medicalLaboratory.type.store") }}');
        $('#TypeId').val('');
    ...
    $('#TypeForm').on('submit', function(e) {
        e.preventDefault();
        let url = $('#TypeId').val() ? '{{ route("medicalLaboratory.type.update", ":id") }}'.replace(':id', $('#TypeId').val()) : '{{ route("medicalLaboratory.type.store") }}';
    ...
    function editService(id) {
        console.log(id);
        $.get('{{ route("medicalLaboratory.type.index") }}/edit/' + id, function(data) {
            $('#TypeId').val(data.id);
            $('#name').val(data.name);
    ...
    function deleteService(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("medicalLaboratory.type.index") }}/delete/' + id,
                    method: 'DELETE',
                    headers: {
