...
<script>
  $(document).ready(function() {
    ...
    var table = $('#organizationInventoriesTable').DataTable({
      processing: true,
      serverSide: true,
      ajax: "{{ route('medicalLaboratory.organization-inventories.data') }}",
      columns: [{
          data: 'id',
          name: 'id'
        },
    ...
    $('#organizationInventoryForm').submit(function(e) {
      ...
      const url = id ? `/medical-laboratory/organization-inventories/update/${id}` : `/medical-laboratory/organization-inventories/store`;
      const method = 'POST';
  ...
  function editOrganizationInventory(id) {
    $.get('{{ route("medicalLaboratory.organization-inventories.index") }}/edit/' + id, function(data) {
  ...
  function deleteOrganizationInventory(id) {
    if (confirm('{{ __("Are you sure you want to delete this organization inventory?") }}')) {
      $.ajax({
        url: '/medical-laboratory/organization-inventories/delete/' + id,
        method: 'DELETE',
        headers: {
