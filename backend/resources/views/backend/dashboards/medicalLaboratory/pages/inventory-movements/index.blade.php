...
<script>
  $(document).ready(function() {
    ...
    var table = $('#inventoryMovementsTable').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route("medicalLaboratory.inventory-movements.data", ":inventoryId") }}'.replace(':inventoryId','{{ $inventoryId }}'),      
      columns: [{
          data: 'id',
          name: 'id'
        },
    ...
    $('#inventoryMovementForm').submit(function(e) {
      ...
      const url = id ? `/medical-laboratory/inventory-movements/update/${id}` : `/medical-laboratory/inventory-movements/store`;
      const method = 'POST';
  ...
  function editInventoryMovement(id) {
    $.get('{{ route("medicalLaboratory.inventory-movements.edit", ":inventory_movement_id") }}'.replace(':inventory_movement_id',id), function(data) {
  ...
  function deleteInventoryMovement(id) {
    if (confirm('{{ __("Are you sure you want to delete this inventory movement?") }}')) {
      $.ajax({
        url: '{{ route("medicalLaboratory.inventory-movements.delete", ":inventory_movement_id") }}'.replace(':inventory_movement_id',id),
        method: 'DELETE',
        headers: {
