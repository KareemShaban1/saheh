...
<script>
   let typeTable = $('#type_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('radiologyCenter.type.data') }}",
        columns: [{
                data: 'id',
                name: 'id'
            },
    ...
    function resetForm() {
        $('#TypeForm')[0].reset();
        $('#TypeForm').attr('action', '{{ route("radiologyCenter.type.store") }}');
        $('#TypeId').val('');
    ...
    $('#TypeForm').on('submit', function(e) {
        e.preventDefault();
        let url = $('#TypeId').val() ? '{{ route("radiologyCenter.type.update", ":id") }}'.replace(':id', $('#TypeId').val()) : '{{ route("radiologyCenter.type.store") }}';
    ...
    function editService(id) {
        console.log(id);
        $.get('{{ route("radiologyCenter.type.index") }}/edit/' + id, function(data) {
            $('#TypeId').val(data.id);
            $('#name').val(data.name);
    ...
    function deleteService(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("radiologyCenter.type.index") }}/delete/' + id,
                    method: 'DELETE',
                    headers: {
