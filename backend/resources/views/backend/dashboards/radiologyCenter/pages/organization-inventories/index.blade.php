...
<script>
  $(document).ready(function() {
    ...
    var table = $('#organizationInventoriesTable').DataTable({
      processing: true,
      serverSide: true,
      ajax: "{{ route('radiologyCenter.organization-inventories.data') }}",
      columns: [{
          data: 'id',
          name: 'id'
        },
    ...
    $('#organizationInventoryForm').submit(function(e) {
      ...
      const url = id ? `/radiology-center/organization-inventories/update/${id}` : `/radiology-center/organization-inventories/store`;
      const method = 'POST';
  ...
  function editOrganizationInventory(id) {
    $.get('{{ route("radiologyCenter.organization-inventories.index") }}/edit/' + id, function(data) {
  ...
  function deleteOrganizationInventory(id) {
    if (confirm('{{ __("Are you sure you want to delete this organization inventory?") }}')) {
      $.ajax({
        url: '/radiology-center/organization-inventories/delete/' + id,
        method: 'DELETE',
        headers: {
