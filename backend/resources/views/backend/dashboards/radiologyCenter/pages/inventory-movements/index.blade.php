@extends('backend.dashboards.radiology-center.layouts.master')
@section('css')
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#inventoryMovementsTable').DataTable({
      processing: true,
      serverSide: true,
      ajax: '{{ route("radiology-center.inventory-movements.data", ":inventoryId") }}'.replace(':inventoryId','{{ $inventoryId }}'),      
      columns: [{
          data: 'id',
          name: 'id'
        },
    ...
    $('#inventoryMovementForm').submit(function(e) {
      ...
      const url = id ? `/radiology-center/inventory-movements/update/${id}` : `/radiology-center/inventory-movements/store`;
      const method = 'POST';
  ...
  function editInventoryMovement(id) {
    $.get('{{ route("radiology-center.inventory-movements.edit", ":inventory_movement_id") }}'.replace(':inventory_movement_id',id), function(data) {
  ...
  function deleteInventoryMovement(id) {
    if (confirm('{{ __("Are you sure you want to delete this inventory movement?") }}')) {
      $.ajax({
        url: '{{ route("radiology-center.inventory-movements.delete", ":inventory_movement_id") }}'.replace(':inventory_movement_id',id),
        method: 'DELETE',
        headers: {
