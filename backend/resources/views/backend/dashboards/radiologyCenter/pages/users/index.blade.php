...
<script>
   let usersTable = $('#users_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('radiologyCenter.users.data') }}",
        columns: [{
                data: 'id',
                name: 'id'
            },
    ...
    function resetForm() {
        $('#userForm')[0].reset();
        $('#userForm').attr('action', '{{ route("radiologyCenter.users.store") }}');
        $('#userId').val('');
    ...
    $('#userForm').on('submit', function(e) {
        e.preventDefault();
        let url = $('#userId').val() ? '{{ route("radiologyCenter.users.update", ":id") }}'.replace(':id', $('#userId').val()) : '{{ route("radiologyCenter.users.store") }}';
    ...
    function editUser(id) {
        console.log(id);
        $.get('{{ route("radiologyCenter.users.index") }}/edit/' + id, function(data) {
            $('#userId').val(data.id);
            $('#name').val(data.name);
    ...
    function deleteUser(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("radiologyCenter.users.index") }}/delete/' + id,
                    method: 'DELETE',
                    headers: {
