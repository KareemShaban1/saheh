		...
		<div class="text-center">
			...
			<div class="flex justify-center">
				<a href="#radiology-centers"
					...
					View All Radiology Centers <i class="fas fa-arrow-down ml-2"></i>
				</a>
			</div>
		</div>
	</div>
...
<section id="radiology-centers" class="py-20">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		@if($radiologyCenters->count() > 0)
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
			@foreach($radiologyCenters as $radiologyCenter)
			<div
				...
				<div class="p-6">
					<div class="flex items-center mb-4 gap-4">
						...
						<div>
							<h3 class="text-xl font-bold text-gray-900">
								{{ $radiologyCenter->name }}</h3>

						</div>
					</div>

					<p class="text-gray-600 mb-4 line-clamp-3">
						{{ Str::limit($radiologyCenter->description, 120) }}</p>

					<div class="space-y-2 mb-4">
						<div class="flex items-center text-gray-600 gap-2">
							<i
								class="fas fa-map-marker-alt mr-2 text-blue-600"></i>
							<span class="text-sm">{{ $radiologyCenter->address }},
								{{ $radiologyCenter->area->name ?? '' }}</span>
						</div>
						<div class="flex items-center text-gray-600 gap-2">
							<i class="fas fa-phone mr-2 text-blue-600"></i>
							<span
								class="text-sm">{{ $radiologyCenter->phone }}</span>
						</div>
					...
					<div class="flex items-center justify-between">
						<div class="flex items-center">
							...
							<span class="text-sm text-gray-600">({{ $radiologyCenter->reviews->count() }}
								reviews)</span>
						</div>
						<span
							class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-semibold">
							{{ $radiologyCenter->is_active ? 'Active' : 'Inactive' }}
						</span>
					</div>
				</div>
				...
				<div class="px-6 pb-6">
					<div class="flex space-x-3">
						<a href="{{ route('radiology-center.detail', $radiologyCenter->id) }}"
						...
						<a href="tel:{{ $radiologyCenter->phone }}"
		...
		<div class="text-center py-16">
			...
			<h3 class="text-2xl font-semibold text-gray-900 mb-4">No Radiology Centers Available
			</h3>
			...
			<a href="{{ route('register-radiology-center') }}"
				...
				<i class="fas fa-plus mr-2"></i> Register Your Radiology Center
			</a>
		</div>
		@endif
	</div>
...
<section class="bg-gray-50 py-16">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="text-center">
			...
			<p class="text-xl text-gray-600 mb-8">Are you a healthcare provider? Register your
				radiology center
			...
			<div class="flex flex-col sm:flex-row gap-4 justify-center">
				<a href="{{ route('register-radiology-center') }}"
					...
					<i class="fas fa-hospital mr-2"></i> Register Radiology Center
				</a>
				<a href="#contact"
