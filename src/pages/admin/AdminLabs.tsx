...
export default function AdminLabs() {
  ...
  const [activeLabId, setActiveLabId] = useState<string | number | null>(null);
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["admin", "labs"],
    queryFn: () => adminApi.medicalLabs({ per_page: "100" }),
  });

  const rows = useMemo<LabRow[]>(() => {
    const root = (data as { data?: unknown })?.data ?? data;
    return ((root as { data?: Array<Record<string, unknown>> })?.data ?? []).map((lab) => ({
      id: String(lab.id ?? "—"),
      name: String(lab.name ?? "—"),
      status: Number(lab.status ?? 1),
      users_count: Number(lab.users_count ?? 0),
      patients_count: Number(lab.patients_count ?? 0),
    }));
  }, [data]);

  const detailsQuery = useQuery({
    queryKey: ["admin", "medical-lab", activeLabId],
    queryFn: () => adminApi.medicalLab(String(activeLabId)),
    enabled: detailsOpen && !!activeLabId,
  });

  const createMutation = useMutation({
    mutationFn: () =>
      adminApi.createMedicalLab({
        name: form.name.trim(),
    ...
    onSuccess: async () => {
      ...
      toast({ title: "Medical laboratory created successfully" });
      setCreateOpen(false);
      setForm({ name: "", email: "", phone: "", address: "", website: "", description: "", status: "active" });
    },
    onError: (e) =>
      toast({
        title: "Failed to create medical laboratory",
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string | number; status: "active" | "inactive" }) => adminApi.updateMedicalLabStatus(id, status),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "labs"] });
      toast({ title: "Medical laboratory status updated" });
    },
    onError: (e) =>
      toast({
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string | number) => adminApi.deleteMedicalLab(id),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "labs"] });
      toast({ title: "Medical laboratory deleted successfully" });
    },
    onError: (e) =>
      toast({
        title: "Failed to delete medical laboratory",
        description: e instanceof Error ? e.message : "Unknown error",
        variant: "destructive",
      }),
  });
  ...
  const toggleStatus = (row: LabRow) => {
    const next = row.status === 1 ? "inactive" : "active";
    statusMutation.mutate({ id: row.id, status: next });
  };

  const deleteLab = (row: LabRow) => {
    const ok = window.confirm(`Delete medical laboratory "${row.name}"? This cannot be undone.`);
    if (!ok) return;
  ...
  const createLab = () => {
    if (!form.name.trim()) {
      toast({ title: "Lab name is required", variant: "destructive" });
      return;
    }
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold">Medical Laboratories</h2>
        </div>
      ...
      {isLoading ? <div className="text-sm text-muted-foreground">Loading medical labs...</div> : null}
      {error ? <div className="text-sm text-destructive">{error instanceof Error ? error.message : "Failed to load medical labs"}</div> : null}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                  ...
                  <tr key={String(row.id)} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button variant="destructive" size="sm" onClick={() => deleteLab(row)} disabled={deleteMutation.isPending}>
                          Delete
                        </Button>
                      </div>
                    </td>
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td className="p-4 text-muted-foreground" colSpan={6}>No medical labs found.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>Add Medical Laboratory</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={createLab} disabled={createMutation.isPending}>{createMutation.isPending ? "Saving..." : "Save"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader><DialogTitle>Medical Laboratory Details</DialogTitle></DialogHeader>
