...
export default function ClinicRegisterPage() {
  ...
  const [specialties, setSpecialties] = useState<{ id: number; name_en?: string; name_ar?: string; name?: string }[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchPublicCollection("/public/specialties"), fetchPublicCollection("/public/governorates")]).then(([sd, gd]) => {
      const parsedSpecialties = Array.isArray(sd)
        ? (sd as Array<{ id: number; name_en?: string; name_ar?: string; name?: string }>)
        : [];
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              >
                <option value="">
                  {specialtiesLoading ? "Loading specialties..." : t("auth.select")}
                </option>
                {specialties.map((s) => (
                  <option key={s.id} value={s.id}>
                    {lang === "ar"
                      ? (s.name_ar || s.name_en || s.name || `#${s.id}`)
                      : (s.name_en || s.name_ar || s.name || `#${s.id}`)}
                  </option>
                ))}
              </select>
            </div>
