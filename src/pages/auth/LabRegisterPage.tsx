...
export default function LabRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await organizationAuthApi.medicalLaboratoryRegister({
        ...form,
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/30 p-4 py-10">
      <Card className="w-full max-w-lg">
        <CardHeader className="text-center">
          ...
          <CardTitle>{t("auth.labRegister")}</CardTitle>
          <CardDescription>{t("auth.labRegisterDesc")}</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>{t("auth.labName")}</Label>
                <Input name="medical_laboratory_name" value={form.medical_laboratory_name} onChange={handleChange} required />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                <Label>{t("auth.labEmail")}</Label>
                <Input name="medical_laboratory_email" type="email" value={form.medical_laboratory_email} onChange={handleChange} required />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
          ...
          <CardFooter className="flex flex-col gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              {t("auth.hasAccount")}{" "}
              <Link to="/lab/login" className="text-primary font-medium hover:underline">{t("auth.login")}</Link>
            </p>
          </CardFooter>
        </form>
      </Card>
